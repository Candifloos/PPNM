...
struct vector {
	...
	void emplace_back(double x) { data.emplace_back(x); }
...
//	void set_col(int j,vector& cj);
    ...
    void print(std::string s="") const{
		...
		for(int i=0;i<size1();i++){
			for(int j=0;j<size2();j++){
                double printstatement = (*this)[i,j];
                if (approx(printstatement, 0.0)) printf("%10.5g ", 0.0);        //prints 0 if its approx 0 (to avoid 1e-16 stuff)
                else
                printf("%10.5g ", printstatement);
            }
			printf("\n");
		}
	}
